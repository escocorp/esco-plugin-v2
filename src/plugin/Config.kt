...
import plugin.models.ApiConfig
import plugin.models.AppConfig
import plugin.models.DatabaseConfig
import plugin.models.DiscordConfig
import plugin.models.HubConfig
import plugin.utils.Gamemode
import plugin.utils.getResource
import java.util.*
...
class Config {
    ...
    fun load(): AppConfig {
        ...
        val appconfig = AppConfig(
            -1,
            getResource("version")?.readString() ?: "",
            !getEnv("LOKI_API").isEmpty(),
            Gamemode.parseGamemode(getEnv("GAMEMODE")),
            ApiConfig(
                getEnv("BUNDLE_API", "http://localhost:8080/bundles/"),
                getEnv("VPN_API", "http://localhost:3000/ip/"),
                Base64.getEncoder().encodeToString(getEnv("API_AUTH").toByteArray()),
                getEnv("LOKI_API")
            ),
            DiscordConfig(
                getEnv("TOKEN"),
                ...
                getEnv("NSFW_ID"),
                getEnv("CONSOLE_ID"),
                getEnv("OWNER_ROLE_ID")
            ),
            DatabaseConfig(
                getEnv("DB_HOST", "127.0.0.1"),
                getEnv("DB_PORT", "5432").toInt(),
                getEnv("DB_USER", "plugin"),
                getEnv("DB_PASSWORD"),
                getEnv("DB", "mindustry")
            ),
            HubConfig()
