...
fun buyUnits(
    ...
    unitCosts.forEach(
        Consumer { en: ObjectIntMap.Entry<UnitType> ->
            val type = en.key
...
fun showTrace(
    ...
    menu
        .add(Bundle.get("menu.trace.name", p.locale) + "\n" + other.coloredName()) { pl: Player ->
    ...
    menu
        .add(Bundle.get("menu.trace.locale", p.locale) + "\n" + other.locale)
        .row()
...
fun showBanMenu(
    ...
    TextMenu(
        Cons2 { pl: Player, reason: String ->
            if (reason.isEmpty()) {
                pl.sendMessage(Bundle.get("menu.ban.no-reason", pl.locale))
                return@Cons2
            }
            TextMenu(
                Cons2 { pl2: Player, time: String ->
                    if (time.isEmpty()) {
